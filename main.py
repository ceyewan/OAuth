...
def _perform_full_oauth_flow() -> Optional[Tuple[Dict, Dict]]:
    ...
    try:
        ...
        # 步骤 4: 用户授权
        print("[4/5] 等待用户授权...")
        httpd = HTTPServer((REDIRECT_HOST, REDIRECT_PORT), _OAuthCallbackHandler)
        ...
        if e.response: print(f"    - 响应内容: {e.response.text}", file=sys.stderr)
        return None
...
def refresh_access_token(client_info: Dict[str, Any], token_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not all([refresh_token, client_id, token_endpoint]):
        print("[!] 缺少刷新所需的信息 (refresh_token, client_id, or token_endpoint)。", file=sys.stderr)
        return None
